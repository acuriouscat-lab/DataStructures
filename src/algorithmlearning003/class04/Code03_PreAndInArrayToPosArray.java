...
public class Code03_PreAndInArrayToPosArray {
    ...
    private static void process1(int[] pre, int preStart, int preEnd, int[] in, int inStart, int inEnd, int[] pos, int start, int end) {
        if (preStart > preEnd) {
            return;
        }
        if (preStart == preEnd) {
            pos[start] = pre[preStart];
            return;
        }
        pos[end] = pre[preStart];
        int mid = inStart;
        for (; mid <= inEnd; mid++) {
