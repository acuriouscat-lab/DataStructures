...
public class Code06_AOE {
    ...
    public static int minAoe2(int[] x, int[] hp, int range) {
        ...
        for (int i = 0; i < x.length; i++) {
            // 如果最左边的有血的话，找到以i为最左边界的范围
            if (hp[i] > 0) {
                int trigger = i;
                // 找到以 i 为最左边，能扩到 trigger - 1 为中心的范围
                while (trigger < N && x[trigger] - x[i] <= range) {
                    trigger++;
                }
                ans += hp[i];
                aoe(x, hp, i, trigger - 1, range);
            }
        }
        return ans;
    }
    ...
    // 需要在 trigger 为中心 range 为半径，减低 hp[L] 的血量
    public static void aoe(int[] x,int[] hp,int L,int trigger,int range){
        int R = trigger;
        while (R < x.length && x[R] - x[trigger] <= range) {
            R++;
        }
        int hpMin = hp[L];
        for (int i = L; i < R; i++) {
            hp[i] = Math.max(0, hp[i] - hpMin);
        }
    }
